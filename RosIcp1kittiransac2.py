...
#        print(len(self.curr_scan_pts), 'origin')
        model_robust, inliers = ransac(self.curr_scan_pts, LineModelND, min_samples=2,
                               residual_threshold=0.9, max_trials=3)
#        print(type(inliers))
...
#            continue
        if self.for_idx==-10:
           self.curr_scan_pts1= ptstoxyz.pointcloud2_to_xyz_array(data,remove_nans=True)
...
#           z2= self.curr_scan_pts[:,2]
           ...
           ax.scatter(x1,y1,z1,marker="*", label ="Nube de puntos" , s= 0.02 , c= 'blue' )
